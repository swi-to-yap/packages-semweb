...
rdf_create_gc_thread :-
	catch(thread_create(rdf_gc_loop, _,
		      [ alias('__rdf_GC')
		      ]), _, true).
...
rdf_gc(CPU) :-
	statistics(cputime, [CPU0,_]),
	(   rdf_gc_
	->  statistics(cputime, [CPU1,_]),
	    CPU is CPU1-CPU0,
	    rdf_add_gc_time(CPU)
	;   CPU = 0.0
	).
...
rdf_update_duplicates_thread :-
	catch(thread_create(rdf_update_duplicates, _,
		      [ detached(true),
...
rdf_start_load(Graph, Ref, _) :-
	thread_self(Me),
	catch(message_queue_create(Queue),_, Queue = []),
...
rdf_end_load(Ref) :-
	...
	erase(Ref),
	catch((thread_send_message(Queue, done),
	       message_queue_destroy(Queue)),_,true).
